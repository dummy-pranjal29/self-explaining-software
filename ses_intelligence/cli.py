...
from ses_intelligence import initialize, get_runtime_snapshots, reset_runtime_state
from ses_intelligence.architecture_health.engine import HealthEngine
from ses_intelligence.architecture_health.forecasting import ForecastEngine
...
def cmd_forecast(args):
    ...
    for i, pred in enumerate(forecast.get("predictions", [])):
...
def cmd_health(args):
    ...
    health = health_engine.compute(snapshots=snapshots)
...
def cmd_status(args):
    """Show SES status."""
    from ses_intelligence import is_initialized, get_config
